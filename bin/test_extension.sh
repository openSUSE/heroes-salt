...
NON_SLS_PILLAR=$(find pillar -type f | grep -Ev "(/(macros|map)\.jinja|(FORMULAS|valid_custom_grains|common/haproxy/goodbots)\.yaml|\.sls)$|infra/(((alerts|certificates)/)?[a-z-]+\.yaml|schemas/\w+\.json)|/README\.md$")
